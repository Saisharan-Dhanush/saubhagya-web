...
const BiogasSanghDashboard: React.FC<DashboardProps> = () => {
  ...
  const setActiveTab = (tab: string) => {
    switch (tab) {
      case 'dashboard':
        ...
        break;
      case 'analytics':
        navigate('/cluster/analytics');
        break;
      case 'maintenance':
        navigate('/cluster/maintenance');
        break;
      case 'farmers':
        navigate('/cluster/farmers');
        break;
    }
  };
  ...
  const getBreadcrumbs = (): BreadcrumbItem[] => {
    ...
    switch (currentTab) {
      case 'dashboard':
        return [
          { label: 'BiogasSangh', onClick: () => navigate('/cluster') },
          { label: 'Production Dashboard' }
        ];
      case 'analytics':
        return [
  ...
  return (
    <BaseLayout
      ...
      moduleSubtitle="Cluster management and biogas production monitoring"
      navigationItems={navigationItems}
      breadcrumbs={getBreadcrumbs()}
      userInfo={{
        name: user?.name || "Suresh Patel",
        role: "Cluster Manager",
      }}
    >
      <Routes>
        <Route path="/" element={<DashboardContent />} />
        <Route path="/analytics" element={<div className="p-8">Analytics & Reports - Coming Soon</div>} />
        <Route path="/farmers" element={<div className="p-8">Farmer Network Management - Coming Soon</div>} />
        <Route path="/maintenance" element={<div className="p-8">System Maintenance - Coming Soon</div>} />
      </Routes>
    </BaseLayout>
  );
};
