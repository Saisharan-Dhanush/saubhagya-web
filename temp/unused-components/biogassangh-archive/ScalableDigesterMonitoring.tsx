...
// Mock hook for digester data - scalable for any number of digesters
const useDigesterData = () => {
  const [digesters] = React.useState<DigesterData[]>([
    ...
    {
      id: 'D004',
      name: 'Digester Beta-1',
      status: 'active',
      temperature: 36.8,
      pressure: 1.3,
      methaneLevel: 72,
      dailyProduction: 165.0,
      efficiency: 90,
      lastUpdated: new Date().toISOString(),
      trends: { methane: 3, production: 4 }
    },
    {
      id: 'D005',
      name: 'Digester Beta-2',
      status: 'active',
      temperature: 34.2,
      pressure: 1.1,
      methaneLevel: 61,
      dailyProduction: 140.0,
      efficiency: 82,
      lastUpdated: new Date().toISOString(),
      trends: { methane: 1, production: 2 }
    }
...
export const ScalableDigesterMonitoring: React.FC<DigesterMonitoringProps> = ({ languageContext }) => {
  ...
  // Dynamic grid class based on number of digesters
  const getDigesterGridClass = (count: number) => {
    if (count <= 2) return 'grid gap-6 md:grid-cols-1 lg:grid-cols-2';
    if (count <= 3) return 'grid gap-6 md:grid-cols-2 lg:grid-cols-3';
    if (count <= 4) return 'grid gap-6 md:grid-cols-2 lg:grid-cols-4';
    if (count <= 6) return 'grid gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6';
    return 'grid gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5';
  };
    ...
    <div className="space-y-6 p-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="overview" className="space-y-6">
          {/* Digesters Grid - Dynamic responsive layout */}
          ...
          {/* Comparison Charts - Dynamic for any number of digesters */}
            ...
            <div className="grid gap-6 lg:grid-cols-2">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Daily biogas production across {digesters.length} digesters
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <DynamicComparisonChart
...
export default ScalableDigesterMonitoring;